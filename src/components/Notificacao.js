...
export default function Notificacao({ data }) {
  ...
  if (data?.data?.type == "application_status") {
    mensagem = data?.data?.message || "Sem mensagem";
    dataEnvio = new Date(data?.created_at).toLocaleString("pt-BR");
  } else if (data?.data?.type == "message_received") {
